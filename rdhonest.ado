...
/*
	...
	1. syntax varlist: depvar [varlist1] (varlist2=varlist_iv) [weight] [if] [in], options 
		
			varlist1: covriates and supplied variance -- sigma2 
			varlist2: treatment variable
			varlist_iv: running variable
			weight: for each obs 
	
	2. options kept:                            Explanations:
					m(real 0)					smooth bound constant 
												by default use ROT for optimal m
												so it's an optional option
					
					c(real 0)   				the cutoff point
					
					kernel(string)     			kernel function type. 
												(uniform, triangular, epanechnikov)
					
					opt_criterion(string)		optimal h by RMSE,FLIC,OIC criterion
					
					alpha(real 0.05)            CI coverage Pr
					
					beta(real 0.8)				for one-sided CI 
					
					se_method(string)			methods calculating standard errors 
												Notes: only NN and EHW
													   it can use sigma2 if sigma2 supplied
					j(integer 3)				parameters for NN 
					
					order(integer 1)            polynomial order. order=1 means only intercept 
					
	   options removed:
					hp(real 0) 					bandwidth above the cutoff 
					
					bw_equal(integer 1)			h above the cutoff == h below the cutoff
					
					hm(real 0)					bandwidth below the cutoff 
					
					sclass(string)              smooth class type 
					
					se_initial(string) 			prelim estimates of var for optimal h 
					                            EHW variance only
												Use IK bandwidth, and compute regression residuals for
												obs within the bandwidth. Then take average squared
												residual above and below the cutoff. This is the
												homoskedastic (on either side of cutoff) variance.
	   options added:
					h(real 0)                   bandwidth for bothsides 
					                            by default use optimal h 
												
					sigma2(integer 1) 		    by default use Var(error term^2|X) 
					
					                         
					
	   options planing:	
					clustervar                  variable to cluster on
...
/*CYG: 1 syntax should be modified to accommodate changes
       
	   2 note m(real 0) is an optional option
	   3 other option changes are described above */	
...
/*CYG: changes made for options are illustrated above*/
  
  /* set defaults */
  if (`hm' == 0 & `bw_equal' == 1) local hm = `hp'              /*CYG: remove*/ // default to equate hm and hp
  if ("`kernel'" == "") local kernel "tri"
  if ("`opt_criterion'" == "") local opt_criterion "MSE"
  if ("`se_method'" == "") local se_method "nn"
  if ("`sclass'" == "") local sclass "H"                        /*CYG: remove*/
  if ("`se_initial'" == "") local se_initial "IKEHW"            /*CYG: remove*/  
...
/*CYG: changes made for options and syntax are illustrated above*/	
    ...
    /* initialize options */
    m = `m'
    bw_equal = `bw_equal'                 /*CYG: remove*/
    kernel = "`kernel'"
    hp = `hp';  hm = `hm'                 /*CYG: remove*/
    opt_criterion = "`opt_criterion'"
    alpha = `alpha'
    beta = `beta'
    se_method = "`se_method'"
    j = `j'
    sclass = "`sclass'"                   /*CYG: remove*/
    order =  `order'
    se_initial = "`se_initial'"           /*CYG: remove*/
	...
	*/
	...
	/*CYG: options are modified
	       m,kernel,h,opt_criterion,
        alpha,beta,se_method,j,order,sigma2
	*/
  ...
  di ""
  di "Call: `y' is dependent variable; `x' is running variable."    /*CYG: modify*/
  ...
  di ""
  di "Bandwidth below cutoff: " hm                                  /*CYG: remove*/
  di "Bandwidth above cutoff: " hp                                  /*CYG: remove*/
  di "Number of effective observations: " effObs
  /*CYG: di "Bandwidth: " h */                                      /*CYG: add*/
  ...
  class RDLPregOutput scalar RDLPreg(class RDData scalar df, real scalar hp,
    ...
    if(strpos(se_method,"supplied_var") > 0) {
      ...
      rm = LPReg(Xm,Ym,hm,kernel,order,se_method,j,sigma2m)
       ...
       //printf("{red}rm.var has value %9.0g \n", rm.var[1])
	   //printf("{red}rm.var has value %9.0g \n", rp.var[1])
    }
    else {
