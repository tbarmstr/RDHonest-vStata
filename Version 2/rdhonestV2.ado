...
program define rdhonestV2, eclass
...
*********************************************
 ...
 set matastrict on
  ...
  mata{
    ...
    Y = st_data(.,("`y'"));
	X = st_data(.,("`runv'"))	
	if (!`Srd') Y = st_data(.,("`y'","`trtmnt'"));
	if (!`NoCovar') Z = st_data(.,("`covar'"));	
	...
	if (`SelfPreVar') sigma2 = st_data(.,("`sigma2'")); 
	if (!`SelfPreVar') sigma2 = J(rows(Y),cols(Y)^2,.);
	if (`Cluster') cluster = st_data(.,("`cluster'"));
	...
	Y = select(Y,sample:==1);
	X = select(X,sample:==1)
	if (!`NoCovar') Z = select(Z,sample:==1)
	...
	df = RDDataPrep(id,X,Y,`c',sigma2,weight,cluster,Class)
	...
	/* generate the wgt vector for each obs in dataset */
    if ("`est_w'"!="NA") ret = RDHonest_fit(df,opt,kernC,"`est_w'",ID,sample)
...
`opt_criterion' `se_method' `kernel' effObs Leverage M2 fs M1 `trtmnt' 
...
program define Display
  ...
  args y runv Srd alpha beta estimate bias stdErr lEqLimit ///
	   ...
	   kernel effObs Leverage M2 fs M1 trtmnt
  ...
  if ("`opt_criterion'"=="OCI") di "{txt} OCI with beta " as result `beta'                                  
...
class RDData scalar RDDataPrep(real vector ID, real matrix X, real matrix Y, real scalar c , ///
	...
	df.setup(ID,X,Y,c,sigma2,weight,cluster,Class)
...
struct RDResults scalar RDHonest_fit (class RDData scalar df,
   ...
   results =  NPRDHonest_fit(df, opt,kernC, 0)
	...
	/* gen a variable from the option est_w */
	 if (args()== 6) {
	 	
		
		regoutput = RDLPreg(df, results.h, opt.kernel, opt.order,  "ehw", ///
		                    1, opt.j)
		...
		/* put kw into orginal sample to get vector est_w */
		kw = sort((df.ID,kw),1)
